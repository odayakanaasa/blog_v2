var page = localStorage.getItem("dragon2") | parseInt( $("#get_page").val() ),
    page_count = 40,
    title = '龙族2';
    to_page();
...
function to_page() {
    ...
    // 每一段区域的解析规则如下
    // 1~60
    pic_src_prefix = 'http://mhpic.zymkcdn.com/comic/L/龙族Ⅱ/' + give_zero(page) + '话/';
    ...
    if (window.localStorage) {
        if (localStorage.getItem("dragon2")) {
            $("#just_look").html('你上次看到第' + localStorage.getItem("dragon2") + '话');
        }
        localStorage.setItem("dragon2", page);
    }
    $("#get_page").val(page);
}
