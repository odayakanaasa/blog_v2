var page = localStorage.getItem("dragon1") | parseInt( $("#get_page").val() ),
    page_count = 40,
    title = '龙族1';
    to_page();
...
function to_page() {
    ...
    // 1~ 44
        pic_src_prefix = 'http://mhpic.zymkcdn.com/comic/L/龙族Ⅰ/' + give_zero(page) + '话/';
    ...
    if (window.localStorage) {
        if (localStorage.getItem("dragon1")) {
            $("#just_look").html('你上次看到第' + localStorage.getItem("dragon1") + '话');
        }
        localStorage.setItem("dragon1", page);
    }
    $("#get_page").val(page);
}
