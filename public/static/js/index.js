	...
	function check_and_toggle(){
		var now = scroll_y % position_id.length;
		position_id.forEach(function(v,index){
			if( index==now ){
				$("body").html('');
				var name = position_id[now]
					,tpl = $("#"+name+"_tpl").html();
				$("body").html( tpl );
				// run app
				eval(name+"();");
