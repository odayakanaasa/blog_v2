<?php
//+++++++++++++++++++++++++++++++++++++++++++++++++++++
//++    SEO良好的分页类  Link: http://www.hlzblog.top/
//+++++++++++++++++++++++++++++++++++++++++++++++++++++
// 用法，见文末
namespace Mine;
use think\Db;
Class Page{
	...
	* @param String : sql_tpl 	sql语句， Db:query方法的传参方式，但是不传 limit的两个参数
	...
	*/
	private function count(){
		if( preg_match('/Select(.*?)(From.*?)Limit/is', $this->sql_tpl, $matches) ){
			...
			$result = Db::query($count_sql, $this->data);
			$this->total = $result[0]['total'];
			$this->page_count =  ceil( $this->total /$this->page_size );
		}else{
			exit("sql语句不匹配。请以：Select(.*?)(From.*?)Limit的形式去书写");
		}
	}
	...
	*/
	private function select(){
		...
		else if( $this->page_count <= $this->to_page ){
		...
		return Db::query($this->sql_tpl, $this->data);
	}
	...
	*/
	private function js(){
		if( $this->page_count == 1 ){ // 如果只有一页，返回空
			return '';
		}
		$source = <<<js_source
...
/* 后台示例：   limit部分，不用写对应的数据，如下
	$p = new \Mine\Page('
				...
				Limit ?,?
    ',[]);
    $p->page_size = 2; // 每页显示的数据条数，默认 15
    $page_data = $p -> get_result();
    $this->assign( 'render', &$page_data["render"] );
    $this->assign( 'info'  , &$page_data["info"] ) ;
*/
